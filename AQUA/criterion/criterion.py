...
from .matcher.box_util import box_cxcywh_to_xyxy, generalized_box_iou
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, src_logits, target_classes_o, idx, num_boxes):
        ...
        if self.loss_class_type == "ce_loss":
            ...
            # advanced indexing으로 직접 1을 할당
            batch_size, num_queries, _ = src_logits.shape
            batch_idx = torch.arange(batch_size, device=src_logits.device).unsqueeze(1).expand(-1, num_queries)
            query_idx = torch.arange(num_queries, device=src_logits.device).unsqueeze(0).expand(batch_size, -1)

            # 정상적인 클래스 번호만 one-hot 처리
            valid_mask = (target_classes != self.num_classes)  # EOS 무시 (num_classes는 EOS에 해당)

            # 조건 필터링
            target_classes_valid = target_classes[valid_mask]
            batch_idx_valid = batch_idx[valid_mask]
            query_idx_valid = query_idx[valid_mask]

            # one-hot 직접 할당
            target_classes_onehot[batch_idx_valid, query_idx_valid, target_classes_valid] = 1
            loss_class = (
                sigmoid_focal_loss(
                    src_logits,
    ...
    def loss_boxes(self, src_boxes, target_boxes, num_boxes):
